...
function drawAnimatedSpriteImpact(_game, ctx, eff, ox, oy) {
  ...
  drawAnimatedSpriteFrame(ctx, {
    ...
    angle: (sprite.rotateWithVelocity ? angle : 0) + (sprite.baseAngleRad || 0),
    elapsed: eff?.t || 0,
    flipY: eff?.flipY === true
  });
}

function drawEnemyBreathVisual(_game, ctx, eff, ox, oy) {
  ...
  const total = Math.max(0.01, Number(eff?.duration) || 0.9);
  const flipY = (Number(eff?.dirX) || 0) < 0;
  ...
  if (elapsed < startDuration && eff.fireStartSprite) {
    const frameIndex = Math.min(
      Math.max(1, eff.fireStartSprite.frameCount) - 1,
      Math.floor((elapsed / Math.max(0.001, startDuration)) * Math.max(1, eff.fireStartSprite.frameCount))
    );
    drawAnimatedSpriteFrame(ctx, {
      image: getAnimatedSpriteImage(eff.fireStartSprite.path),
      sprite: eff.fireStartSprite,
      centerX,
      centerY,
      angle,
      frameIndex,
  ...
  if (elapsed >= endStart && eff.fireEndSprite) {
    const frameIndex = Math.min(
      Math.max(1, eff.fireEndSprite.frameCount) - 1,
      Math.floor(((elapsed - endStart) / Math.max(0.001, total - endStart)) * Math.max(1, eff.fireEndSprite.frameCount))
    );
    drawAnimatedSpriteFrame(ctx, {
      image: getAnimatedSpriteImage(eff.fireEndSprite.path),
      sprite: eff.fireEndSprite,
      centerX,
      centerY,
      angle,
      frameIndex,
  ...
  if (eff.fireLoopSprite) {
    drawAnimatedSpriteFrame(ctx, {
      image: getAnimatedSpriteImage(eff.fireLoopSprite.path),
      sprite: eff.fireLoopSprite,
      centerX,
      centerY,
      angle,
      elapsed,
...
export const SKILL_EFFECT_DRAW_HANDLERS = {
  ...
  enemyBreathVisual: drawEnemyBreathVisual,
